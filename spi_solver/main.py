...
def main(screenshot_dir: str | None) -> None:
    if screenshot_dir is None:
        if platform.system() == "Windows":
            screenshot_dir = os.path.join(
                os.environ["USERPROFILE"], "Pictures", "Screenshots"
            )
        else:
            # TODO: enable setting the default screenshot dir using env var
            raise ValueError(
                "Please provide the path to the directory to watch for new images."
            )
    ...
    thread = threading.Thread(
        target=start_watching, args=(screenshot_dir, image_queue), daemon=True
    )
    thread.start()
